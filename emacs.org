...
As of now, <2023-03-19 Sun>, python mode is not included in prog-mode so function is also added to python mode in the python section below.
...
#+begin_src emacs-lisp
  ...
  (if (version<= "29.51" emacs-version)
      (with-eval-after-load 'org
...
#+begin_src emacs-lisp
  ...
  (defun jmn-load-gruvbox-dark-hard ()
    ...
    ;; (set-face-background 'default "black") ;; black shows up blue in term
    (set-face-foreground 'default "bisque2") ;; default "#ebdbb2"

    (set-face-background 'mode-line-inactive "black")
    (set-face-background 'mode-line-active   "gray30")
...
* Hacky Fixes
** removes following error: File mode specification error: (error Invalid image type ‘svg’)
#+begin_src emacs-lisp
...
** TODO Emacs 30 get "???" instead of inbox in agenda
Saving accessing and saving inbox.org will fix the issue.
